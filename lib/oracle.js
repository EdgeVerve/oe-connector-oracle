...
Oracle.prototype.executeSQL = function (sql, params, options, callback) {
  ...
  function executeWithConnection(connection, newOptions, release) {
    ...
    connection.execute(sql, params, newOptions, function (err, data) {
      ...
      if(err){
        console.log("Oracle error ", err, sql, params);
      }
  ...
  for (var key in where) {
    ...
    if (expression === null || expression === undefined) {
      ...
      if (operator === 'like' || operator === 'nlike' || operator === 'ilike' || operator === 'nilike') {
        this._expression = expression = expression[operator];
...
Oracle.prototype._buildFieldsForKeys = function (model, data, keys, excludeIds) {
  ...
  // Object.keys(props).reduce((carrier, key) => {
  //   let propDef = props[key];
  //   if('oracle' in propDef && 'sequence' in propDef.oracle && )
  // });
  let propNames = Object.keys(props);
  let complexSeqFieldIndex = propNames.findIndex(key => {
    let propDef = props[key];
    return 'oracle' in propDef && 'sequence' in propDef.oracle && propDef.oracle.sequence.type === 'complex';
  });
  if(complexSeqFieldIndex > -1) {
    let fieldName = propNames[complexSeqFieldIndex]
    let def = props[fieldName];
    fields.names.push(fieldName);
    fields.columnValues.push(this.toColumnValue(def, null));
    fields.properties.push(props[fieldName])
  }
...
Oracle.prototype.toColumnValue = function (prop, val) {
  if (val == null) {
    ...
    if(prop.oracle && prop.oracle.sequence && prop.oracle.sequence.type === 'complex') {
      let { length, prefix, name } = prop.oracle.sequence;
      let strLength = length - prefix.length;
      return new ParameterizedSQL(`'${prefix}' || LPAD( TO_CHAR(${name}.nextval), ${strLength}, '0')`)
    }
    ...
    switch (operator) {
      ...
      case 'contains':
        ...
        if (Array.isArray(propertyDefinition.type)) {
          ...
          expr.sql = "oe_contains(?, NVL(" + columnName + ", '[]')) = 'true'";
        }
        return expr;
      case 'nin':
        var operatorValue = columnValue;
...
Oracle.prototype.destroyAll = function destroyAll(model, filter, options, cb) {
  ...
  if (self.typesCreated)
    ...
    self.createTypes(function () {
      self.updateAndCall(self.innerdestroyAll, args);
    });
  }
}
Oracle.prototype.save = function save(model, data, options, cb) {
  ...
  if (self.typesCreated)
    ...
    self.createTypes(function () {
      self.updateAndCall(self.innersave, args);
    });
  }
}
...
Oracle.prototype.update = function update(model, where, data, options, cb) {
  ...
  if (self.typesCreated)
    ...
    self.createTypes(function () {
      self.updateAndCall(self.innerupdate, args);
    });
  }
}
...
Oracle.prototype.count = function count(model, where, options, cb) {
  ...
  if (self.typesCreated)
    ...
    self.createTypes(function () {
      self.updateAndCall(self.innercount, args);
    });
  }
}
...
// overriding as we have to check if table exists manually
Oracle.prototype.dropTable = function(model, cb) {  
  let tableName = this.dbName(model);
  this.execute(`DROP TABLE ${tableName}`, [], {}, function(err){
