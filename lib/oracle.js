...
Oracle.prototype.dropTable = function(model, cb) {
  let existsQuery = `SELECT COUNT(*) AS has_table from USER_TABLES where TABLE_NAME = '${model}'`;
  var self = this;
  self.execute(existsQuery, [], {}, function(err, results) {
    if(err) {
      cb(err);
    }
    else {
      let [record] = results;
      if(record.HAS_TABLE) {
        let dropQuery = `DROP TABLE ${model}`;
        self.execute(dropQuery, [], {}, cb);
      }
      else {
        cb();
      }
