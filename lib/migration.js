        ...
        if (Array.isArray(prop.type) && !prop.type.name && prop.type[0].name) {
          ...
          switch (tempColumnType) {
            ...
            default:
              ...
              if(constraintName.length>30){
                ...
                */
                constraintName = model + '_' + uuid.v4();
                constraintName = constraintName.substr(0, 30);
                console.log('New Constraint Name 1: ', constraintName);
              }              
          ...
          if(constraintName.length>30){
            ...
            */
            constraintName = model + '_' + uuid.v4();
            constraintName = constraintName.substr(0, 30);
            console.log('New Constraint Name 2: ', constraintName);
          }          
          return "CLOB CONSTRAINT \"" + constraintName + "\" CHECK (\"" + property.toUpperCase() + "\" IS JSON FORMAT JSON)";
        }
      case 'String':
      ...
      case 'JSON':
        ...
        if(constraintName.length>30){
          ...
          */
          constraintName = model + '_' + uuid.v4();
          constraintName = constraintName.substr(0, 30);
          console.log('New Constraint Name 3: ', constraintName);
        }        
        return "CLOB CONSTRAINT \"" + constraintName + "\" CHECK (\"" + property.toUpperCase() + "\" IS JSON FORMAT JSON)";
      case 'Text':
        return 'CLOB';
  ...
  Oracle.prototype.addIndexes = function (model, actualIndexes, cb) {
    ...
    propNames.forEach(function (propName) {
      ...
      if(iName.length>30){
         ...
         * Use uuid.v4 for such cases
        */
        iName = self.table(model) + '_' + uuid.v4();
