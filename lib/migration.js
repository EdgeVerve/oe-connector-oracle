...
const SEQ_COMPLEX = Object.assign({}, SEQ_SIMPLE, {
  name: null,       // sequence name - required
  length: 0,        // final length of prefix-ed sequence
  prefix: null      // the prefix to appear before the padded sequence
});

function checkSequence(self, seqDef, cb) {
  let seqQuery = `SELECT count(*) as hasSequence from user_sequences where sequence_name = '${self.dbName(seqDef.name)}'`;
  self.executeSQL(seqQuery, [], {}, function(err, results) {
    if(err) {
      cb(err);
    }
    else if(results.length === 1 && results[0]['HASSEQUENCE']) { // --__(*.*)__--
      cb(null, true)
    }
    else {
      cb(null, false);
...
function createSequence(self, modelName, seqDef, cb) {
  ...
  let createSequenceStatement = 'CREATE SEQUENCE ';
  ...
  if (seqDef.name === 'null' && typeof seqDef.name !== 'string') {
    ...
    createSequenceStatement += seqDef.name + NEWLINE;
  }
  ...
  if (typeOfMinVal === 'boolean' && minValue === false) {
    createSequenceStatement += 'NOMINVALUE' + NEWLINE;
  }
  ...
  if (typeOfMaxVal === 'boolean' && maxValue === false) {
    createSequenceStatement += 'NOMAXVALUE' + NEWLINE;
  }
  ...
  if (typeOfStartFrom === 'number') {
    createSequenceStatement += `START WITH ${startFrom}` + NEWLINE;
  }
  else {
  ...
  if (typeOfCache === 'number' && cache >= 2) {
    createSequenceStatement += `CACHE ${cache}` + NEWLINE;
  }
  else if (typeOfCache === 'boolean' && cache === false) {
    createSequenceStatement += 'NOCACHE' + NEWLINE;
  }
  else if (typeOfCache === 'boolean') {
    createSequenceStatement += 'CACHE 2' + NEWLINE;
  }
  else {
    return cb(new Error('Invalid sequence definition, "cache" should be a number, and greater than or equals to 2, OR, a boolean'));
  }
  ...
  if (typeOfCycle === 'boolean' && cycle) {
    ...
    createSequenceStatement += 'NOCYCLE' + NEWLINE;
  }
  else {
  ...
  //order
  let { order } = seqDef;
  let typeOfOrder = typeof order;
  if(typeOfOrder === 'boolean' && order) {
    createSequenceStatement += 'ORDER' + NEWLINE;
  }
  else if(typeOfOrder === 'boolean') {
    createSequenceStatement += 'NOORDER' + NEWLINE;
  }
  else {
    return cb(new Error('Invalid sequence definition, "order" should be boolean value'));
  }

  self.executeSQL(createSequenceStatement, [], {}, err => {
    cb(err);
  });
}
...
function doSimpleSequence(connector, modelName, key, propDef, cb) {
  if (propDef.type.name !== 'Number') {
    ...
    let self = connector;
    let { oracle: { sequence } } = propDef;
    let seqDef = Object.assign({}, SEQ_SIMPLE, sequence);    
    createSequence(self, modelName, seqDef, function(err){
...
function doComplexSequence(connector, modelName, key, propDef, cb) {
  if (propDef.type.name !== 'String') {
    ...
    let self = connector;
    let { oracle: { sequence } } = propDef;
    let seqDef = Object.assign({}, SEQ_COMPLEX, sequence);
    createSequence(self, modelName, seqDef, function(err){
      if(err && err.errorNum !== 955) {
        cb(err)
      }
...
var getNewNumber = getNewNumberCtr();
...
function mixinMigration(Oracle) {
  ...
  Oracle.prototype.getAddModifyColumns = function (model, actualFields) {
    var sql = [];
    var self = this;
    var additions = self.getColumnsToAdd(model, actualFields);
    if (additions && additions.length > 0) {
      sql.push('ADD (' + additions.join(',') + ')');
    }
    sql = sql.concat();
    ...
    propNames.forEach(function (propName) {
      ...
      if (found && self.propertyHasNotBeenDeleted(model, propName)) {
        if (datatypeChanged(propName, found)) {
          sql.push(self.modifyDatatypeInActual(model, propName));
        }
        if (nullabilityChanged(propName, found)) {
          sql.push(self.modifyNullabilityInActual(model, propName));
        }
      }
    });
  ...
  Oracle.prototype.buildColumnDefinition = function (model, propName) {
    ...
    if('oracle' in prop && 'sequence' in prop.oracle && prop.oracle.sequence.type === 'simple') {
      let seqDef = prop.oracle.sequence;
      return `NUMBER DEFAULT ${seqDef.name}.nextval`;
    }
  ...
  Oracle.prototype.createTable = function (model, cb) {
    ...
    var sequenceSupportedProperties = Object.keys(properties).reduce((carrier, key) => {
      ...
      if (typeof currentEntry === 'object' && 'oracle' in currentEntry && 'sequence' in currentEntry.oracle) {
          ...
          switch (tempColumnType) {
            ...
            default:
              ...
              if (constraintName.length > 30) {
                /** Simply truncating to 30 characters, sometimes results in duplicate constraint names 
                 * ProcessInstanceHistory._processTokens and ProcessInstanceHistory._processVariables
                 * Both result in 'ProcessInstanceHistory_proces' as constraint name
                 * Use uuid.v4 for such cases
                */
                constraintName = model.substr(0, 25) + '_' + getNewNumber(); //uuid.v4();
                constraintName = constraintName.substr(0, 30);
              }
  ...
  Oracle.prototype.addIndexes = function (model, actualIndexes, cb) {
    ...
    propNames.forEach(function (propName) {
      ...
      if (iName.length > 30) {
        /** Simply truncating to 30 characters, sometimes results in duplicate index names 
         * ProcessInstanceHistory._processTokens and ProcessInstanceHistory._processVariables
         * Both result in 'ProcessInstanceHistory_proces' as index name
         * Use uuid.v4 for such cases
        */
        iName = self.table(model).substr(0, 25) + '_' + getNewNumber(); // uuid.v4();
        iName = iName.substr(0, 30);
      }
    ...
    indexNames.forEach(function (indexName) {
      ...
      if (!found && deletedIndexes.indexOf(indexName) === -1) {
        ...
        if (iName.length > 30) {
          iName = iName.substr(0, 25) + '_' + getNewNumber();
        }
